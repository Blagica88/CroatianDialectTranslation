...
def test_sharing(encoder, decoder, lm, params):
    ...
    if params.share_decpro_emb:
        for i in range(params.n_langs):
            assert_equal(decoder.proj.private_proj[i].weight, decoder.embeddings[i].weight)
        if params.share_lang_emb:
            assert params.share_output_emb
            for i in range(1, params.n_langs):
                assert_equal(decoder.proj.private_proj[i].bias, decoder.proj.private_proj[0].bias)
    elif params.share_output_emb:
        assert params.share_lang_emb
        for i in range(1, params.n_langs):
            assert_equal(decoder.proj.private_proj[i].weight, decoder.proj[0].weight)
            assert_equal(decoder.proj.private_proj[i].bias, decoder.proj[0].bias)
