    ...
    with open(path, encoding='UTF-8') as f:
        for i, line in enumerate(f):
            if i == 0:
                print("tuka")
                split = line.split()
                print(dim, int(split[0]), int(split[1]))
                assert len(split) == 2
...
def initialize_embeddings(encoder, decoder, params, data):
    ...
    if len(split) == 1:
        ...
        pretrained_0, word2id_0 = reload_embeddings(params.pretrained_emb, params.emb_dim - params.shared_emb_dim)
        ...
        word2id = []
        for path in split:
            pretrained_i, word2id_i = reload_embeddings(path, params.emb_dim - params.shared_emb_dim)
    ...
    shared_embs, shared_vocab = reload_embeddings(params.pretrained_shared_emb, params.shared_emb_dim)
    # dico = data['dico'][params.langs[0]]
    # for word_id in range(params.n_words[0]):
    #     word = dico[word_id]
    #     if word in shared_vocab:
    #         vec = torch.from_numpy(shared_embs[shared_vocab[word]]).cuda()
    #         encoder.shared_embedding.weight.data[word_id] = vec
    ...
    for word_id in range(params.n_words[0]):
        ...
        if word in shared_vocab:
            vec = torch.from_numpy(shared_embs[shared_vocab[word]]).cuda()
            encoder.shared_embedding.weight.data[word_id] = vec
            decoder.shared_embedding.weight.data[word_id] = vec
            decoder.proj.shared_proj.weight.data[word_id] = vec
