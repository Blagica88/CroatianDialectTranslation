...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        if args.share_lang_emb:
            ...
            layer_0 = Embedding(args.n_words[0], embed_dim - args.shared_emb_dim, args.pad_index)
            embeddings = [layer_0 for _ in range(self.n_langs)]
        else:
            embeddings = [Embedding(n_words, embed_dim - args.shared_emb_dim, padding_idx=args.pad_index) for n_words in self.n_words]
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, args, encoder):
        ...
        if self.share_encdec_emb:
            ...
            if self.share_lang_emb:
                ...
                layer_0 = Embedding(self.n_words[0], self.emb_dim - args.shared_emb_dim, padding_idx=self.pad_index)
                embeddings = [layer_0 for _ in range(self.n_langs)]
            else:
                embeddings = [Embedding(n_words, self.emb_dim - args.shared_emb_dim, padding_idx=self.pad_index) for n_words in self.n_words]
        ...
        # projection layers
        # proj = [nn.Linear(self.emb_dim, n_words, False) for n_words in self.n_words]
    ...
    def forward(self, encoded, y, lang_id, one_hot=False, incremental_state=None):
        ...
        x = torch.cat((embed_tokens(prev_output_tokens), self.shared_embedding(prev_output_tokens)), dim=-1).mul(self.embed_scale)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.self_attn = MultiheadAttention(
            self.embed_dim, args.encoder_attention_heads, True,
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.self_attn = MultiheadAttention(
            self.embed_dim, args.decoder_attention_heads, True,
            dropout=args.attention_dropout,
        )
        self.dropout = args.dropout
        self.relu_dropout = args.relu_dropout
        ...
        self.encoder_attn = MultiheadAttention(
            self.embed_dim, args.decoder_attention_heads, False,
            dropout=args.attention_dropout,
        )
...
class SharedProjection(nn.Module):
    ...
    def forward(self, embeddings, lang_id):
        lang_projection = self.private_proj[lang_id]

        cat_proj = torch.cat((lang_projection.weight, self.shared_proj.weight), dim=-1)
        return torch.matmul(embeddings, cat_proj.t())
